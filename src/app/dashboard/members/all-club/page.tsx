...
export default function AllClubMembersPanel() {
  ...
  const fetchMembers = async (forceSearch = '') => {
    ...
    try {
      ...
      const res = await api.get<any>(
        `/admin/members?page=${page}&search=${qSearch}`
      );
  ...
  const handleOpenDetail = async (m: Member) => {
    ...
    try {
      ...
      const res = await api.get<any>(`/admin/transactions?branch_id=${m.branch_id}&per_page=100`);
      if (res.success && res.data) {
    ...
    <div className="space-y-8 font-sans">
      ...
      {step === 'list' && (
        <div className="space-y-6">
          <div>
            <h2 className="text-3xl font-heading text-slate-800">ANGGOTA ALL CLUB</h2>
            <p className="text-slate-500 text-sm mt-1 uppercase tracking-widest font-accent">
              Semua Cabang — Daftar Anggota & Riwayat Keuangan Aktif
            </p>
          </div>
          ...
          <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden">
            <div className="bg-[#17A2B8] px-5 py-3 text-white font-bold select-none flex items-center gap-2">
              ...
              <span className="text-sm uppercase tracking-wider">Search All Club</span>
            </div>
            <div className="p-6">
          ...
          {loading ? (
            ...
            <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden">
              <div className="bg-[#17A2B8] px-5 py-3 text-white font-bold select-none">
                <span className="text-sm uppercase tracking-wider">Daftar Anggota All Club</span>
              </div>
              <div className="p-6">
                  ...
                  <table className="w-full text-left text-sm text-slate-650 border border-slate-200">
                    <thead className="bg-[#6C7A89] text-white text-[10px] uppercase tracking-wider font-bold select-none">
                      <tr>
                        ...
                        <th className="py-3 px-4 border-r border-slate-350/40">Cabang</th>
                      ...
                      {members.length > 0 ? (
                        members.map((m, idx) => (
                          <tr key={m.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="py-4 px-4 border-r border-slate-100 text-slate-800 font-bold">{m.branch_name || 'Grogol'}</td>
      ...
      {/* UBahr Data Anggota Step (Image 3 Layout) */}
