...
export default function CSStaffManagementPage() {
  ...
  const handleOpenEdit = (cs: any) => {
    setEditingId(cs.id);
    setUsername(cs.username || '');
    setFullName(cs.full_name || '');
    setEmail(cs.email || '');
    setPhone(cs.phone || '');
    setPassword(cs.password || '');
    setShowFormPassword(false);
    setBranchID(cs.branch_id || (branches[0]?.id || ''));
    setError(null);
    setSuccess(null);
    setStep('edit');
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Gagal menyimpan data Customer Service');
    }
  };

  const handleDelete = async (id: string, name: string) => {
    if (!confirm(`Apakah Anda yakin ingin menonaktifkan CS ${name}?`)) return;
    try {
      await employeesApi.delete(id);
      fetchCSList();
    } catch (err: any) {
      alert(err.response?.data?.message || 'Gagal menghapus Customer Service');
    }
  };

  // State to track password visibility per CS ID
  ...
  const columns: Column<any>[] = [
    ...
    { key: 'username', header: 'Username', render: (row) => <span className="font-mono font-bold text-slate-800">{row.username}</span> },
    {
      ...
      render: (row) => {
        ...
        const displayPass = row.password || 'password123';
        return (
          <div className="flex items-center gap-1.5 font-mono text-xs select-none">
            ...
            <button
              ...
              className="p-1 text-slate-400 hover:text-[#17A2B8] transition-colors cursor-pointer"
    ...
    { key: 'full_name', header: 'Nama Lengkap', render: (row) => <span className="font-bold">{row.full_name}</span> },
    { key: 'branch_name', header: 'Cabang Tugas', render: (row) => <span className="uppercase text-[10px] font-bold text-[#17A2B8]">{row.branch_name}</span> },
        ...
        <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden p-6 space-y-4">
          <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
            ...
            <h3 className="font-heading text-base font-bold text-slate-800 uppercase tracking-wider">
              Daftar Akun Customer Service (CS)
            </h3>
          </div>
      ...
      {(step === 'create' || step === 'edit') && (
        <div className="bg-white border border-slate-200 rounded shadow-sm overflow-hidden max-w-2xl mx-auto">
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-5 text-xs text-slate-700">
            ...
            <div className="space-y-1.5">
              <label className="font-bold text-slate-800 block">Username Akses Login CS</label>
              <input
                type="text"
                value={username}
                onChange={(e) => setUsername(e.target.value)}
                className="w-full bg-slate-50 border border-slate-300 p-2.5 rounded font-mono font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-[#17A2B8]"
                placeholder={step === 'create' ? 'Otomatis dibuat jika dikosongkan (contoh: budi.gunawan1234)' : 'Username CS'}
              />
              <span className="text-[10px] text-slate-400 block">
                Digunakan CS untuk login. Dikosongkan saat pembuatan = dibuat otomatis oleh sistem.
              </span>
            </div>

            <div className="space-y-1.5">
              <label className="font-bold text-slate-800 block">Nama Lengkap CS *</label>
              <input
                type="text"
                value={fullName}
            ...
            <div className="space-y-1.5">
              <label className="font-bold text-slate-800 block">Password Akses Login CS {step === 'create' ? '*' : ''}</label>
              <div className="relative flex items-center">
                <input
                  type={showFormPassword ? 'text' : 'password'}
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  className="w-full bg-slate-50 border border-slate-300 p-2.5 pr-10 rounded font-mono font-bold text-slate-800 focus:outline-none focus:ring-1 focus:ring-[#17A2B8]"
                  placeholder="Masukkan Password CS (Contoh: cs123456)"
                  required={step === 'create'}
                />
                <button
                  type="button"
                  onClick={() => setShowFormPassword(!showFormPassword)}
                  className="absolute right-3 text-slate-400 hover:text-[#17A2B8] p-1 cursor-pointer"
              ...
              <span className="text-[10px] text-slate-400 block">
                {step === 'create' ? 'Password ini digunakan CS untuk login ke sistem' : 'Ubah password jika ingin memperbarui, atau biarkan jika tidak berubah'}
              </span>
            </div>

            <div className="space-y-1.5">
              <label className="font-bold text-slate-800 block">Email CS</label>
              <input
                type="email"
                value={email}
