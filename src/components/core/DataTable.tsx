...
export function DataTable<T extends { id: string | number }>({
  ...
  return (
    <div className={`overflow-hidden w-full max-w-full ${className}`}>
      {title && (
          ...
          <thead className="bg-[#6C7A89] text-white text-[10px] uppercase tracking-wider font-bold select-none">
            <tr>
              {columns.map((col, idx) => (
                <th
                  key={col.key}
                  className={`py-3 px-3 sm:px-4 ${idx < columns.length - 1 ? 'border-r border-slate-350/40' : ''
                ...
                return (
                  <tr key={item.id} className="hover:bg-slate-50/50 transition-colors">
                    {columns.map((col, idx) => (
                      <td
                        key={col.key}
                        className={`py-3 sm:py-3.5 px-3 sm:px-4 ${idx < columns.length - 1 ? 'border-r border-slate-100' : ''
                          } ${col.align === 'center' ? 'text-center' : col.align === 'right' ? 'text-right' : 'text-left'
                          } ${col.className || ''}`}
                      >
            ...
            {pageNumbers.map((p, idx) => {
              ...
              return (
                <button
                  ...
                  className={`px-3 py-1 border rounded text-[11px] font-bold transition-all cursor-pointer ${isActive
